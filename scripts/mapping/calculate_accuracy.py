...
def _usage(str):
    print "Usage:", sys.argv[0], "mapped read file (sam/bam), reference file (fasta)"
    print "Example:", sys.argv[0], "output.bam used_ref.fa"
    print str
    sys.exit(1)
...
#	print samfile.getrname(read.tid)
	...
	if read.is_unmapped:
		...
		for n, base in enumerate(read.seq):
		    if not read.qual: 	
			print >>sys.stderr, "no quality for", read
			continue	
